...
func NewCompare(srcOld any, srcNew any) error {
	...
	if fileName, ok := srcOld.(string); ok {
		...
		return fmt.Errorf("cat not get file source %V", srcOld)
	}
	...
	if fileName, ok := srcNew.(string); ok {
		...
		return fmt.Errorf("cat not get file source %V", srcNew)
	}
	target := ast.File{
		Doc:      newNodes.Doc,
		Name:     oldNodes.Name,
	...
	return coinsOptChain(newNodes, oldNodes, &target, HeaderMerge, VarMerge, ConstsMerge, StructMerge, InterfaceMerge, FuncMerge, ExFuncMerge)
}
