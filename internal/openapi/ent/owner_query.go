...
func (oq *OwnerQuery) Order(o ...owner.OrderOption) *OwnerQuery {
...
func (oq *OwnerQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (oq *OwnerQuery) Clone() *OwnerQuery {
	...
	return &OwnerQuery{
		config:     oq.config,
		ctx:        oq.ctx.Clone(),
		order:      append([]owner.OrderOption{}, oq.order...),
...
func (oq *OwnerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Owner, error) {
	...
	if query := oq.withPets; query != nil {
		if err := oq.loadPets(ctx, query, nodes,
			func(n *Owner) { n.Edges.Pets = []*Pet{} },
			func(n *Owner, e *Pet) { n.Edges.Pets = append(n.Edges.Pets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OwnerQuery) loadPets(ctx context.Context, query *PetQuery, nodes []*Owner, init func(*Owner), assign func(*Owner, *Pet)) error {
	...
	query.Where(predicate.Pet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(owner.PetsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.owner_pets
		if fk == nil {
			return fmt.Errorf(`foreign-key "owner_pets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "owner_pets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (oq *OwnerQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(owner.Table, owner.Columns, sqlgraph.NewFieldSpec(owner.FieldID, field.TypeInt))
	_spec.From = oq.sql
