...
func (pq *PetQuery) QueryCollar() *CollarQuery {
	query := (&CollarClient{config: pq.config}).Query()
...
func (pq *PetQuery) QueryOwner() *OwnerQuery {
	query := (&OwnerClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Pet, error) {
	...
	if query := pq.withCollar; query != nil {
		if err := pq.loadCollar(ctx, query, nodes, nil,
			func(n *Pet, e *Collar) { n.Edges.Collar = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := pq.withOwner; query != nil {
		if err := pq.loadOwner(ctx, query, nodes, nil,
			func(n *Pet, e *Owner) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *PetQuery) loadCollar(ctx context.Context, query *CollarQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *Collar)) error {
	...
	query.Where(predicate.Collar(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(pet.CollarColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.pet_collar
		if fk == nil {
			return fmt.Errorf(`foreign-key "pet_collar" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "pet_collar" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pq *PetQuery) loadCategories(ctx context.Context, query *CategoryQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *Category)) error {
	...
	nids := make(map[uint64]map[*Pet]struct{})
...
func (pq *PetQuery) loadOwner(ctx context.Context, query *OwnerQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *Owner)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "owner_pets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
