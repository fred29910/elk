...
func (iq *ItemQuery) QueryCompartment() *CompartmentQuery {
	query := (&CompartmentClient{config: iq.config}).Query()
...
func (iq *ItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Item, error) {
	...
	if query := iq.withCompartment; query != nil {
		if err := iq.loadCompartment(ctx, query, nodes, nil,
			func(n *Item, e *Compartment) { n.Edges.Compartment = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ItemQuery) loadCompartment(ctx context.Context, query *CompartmentQuery, nodes []*Item, init func(*Item), assign func(*Item, *Compartment)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "compartment_contents" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
